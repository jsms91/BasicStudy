...
public class ex_02 {
    public static void main(String[] args) {
        ...
        for(int i=1; i<n; i++) {
            ...
            //-> Math.pow(i,2)로 시작하든지  or Math.pow(n-i,2)로 해도 된다.

            result += i==1 ? num : num*(start-1); //처음 1일 경우에는 대각선 정사각형이 없고 이후 길이가 2이상일 경우부터는 양똑 점을 제외하고 가운데 점의 개수가 대각선 정사각형의 개수이다.
            //-> 이게 결국 루트씌우는 방식이라서 루트로 구하는 방식으로도 해보기


            start++;//정사각형의 단위 증가(1*1 -> 2*2 -> 3*3 여기서는 점이 2개 있다 길이가 1로 생각해서 문제 풀어서 점이 2개일 경우가 초기값으로 1씩 증가한다.
        }
